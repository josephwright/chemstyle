...
%.pdf: %.dtx
	...
	pdflatex -draftmode -interaction=nonstopmode -shell-escape "\AtBeginDocument{\OnlyDescription} \input $<" &> /dev/null ; \
	if [ $$? = 0 ] ; then  \
	  makeindex -s gglo.ist -o $$NAME.gls $$NAME.glo &> /dev/null ; \
	  makeindex -s gind.ist -o $$NAME.ind $$NAME.idx &> /dev/null ; \
	  pdflatex -interaction=nonstopmode -shell-escape "\AtBeginDocument{\OnlyDescription} \input $<" &> /dev/null ; \
	  makeindex -s gind.ist -o $$NAME.ind $$NAME.idx &> /dev/null ; \
	  pdflatex -interaction=nonstopmode -shell-escape "\AtBeginDocument{\OnlyDescription} \input $<" &> /dev/null ; \
	else \
