...
:init
  ...
  set AUXFILES=aux dvi glo gls hd idx ilg ind ist log los out tmp toc
  set CLEAN=cfg eps gz ins pdf sty tex txt zip
  ...
  set CTANFILES=cdx dtx ins pdf
  set TDSFILES=%CTANFILES% cfg sty tex 
...
:clean-aux
  ...
  if exist *-pics.pdf del /q *-pics.pdf 
...
:file2tdsdir
  ...
  if /i "%~x1" == ".cdx" set TDSDIR=doc\latex\%PACKAGE%
...
:typeset-dtx
  ...
  pdflatex -draftmode -interaction=nonstopmode -shell-escape "\AtBeginDocument{\OnlyDescription}\input %1" > nul
  if ERRORLEVEL 1 (
    echo ! Compilation failed
  ) else (
    makeindex -q -s gglo.ist -o %~n1.gls %~n1.glo > nul
    makeindex -q -s gind.ist -o %~n1.ind %~n1.idx > nul
    pdflatex -interaction=nonstopmode -shell-escape "\AtBeginDocument{\OnlyDescription} \input %1" > nul
    pdflatex -interaction=nonstopmode -shell-escape "\AtBeginDocument{\OnlyDescription} \input %1" > nul
  )
